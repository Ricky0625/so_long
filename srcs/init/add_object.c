...
/*   Updated: 2022/11/22 14:13:42 by wricky-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_vwall(t_game *game, t_vector loc)
{
	...
	anim_init(&vwall->anim, VWALL_AD, game->img_db.vwall_idle);
	vwall->loc = loc;
...
void	add_collectible(t_game *game, t_vector loc)
{
	...
	anim_init(&coll->anim, KEY_AD, game->img_db.key_idle);
	coll->loc = loc;
...
void	add_exit(t_game *game, t_vector loc)
{
	...
	exit->clear = 0;
	exit->loc = loc;
	anim_init(&exit->anim, EXIT_AD, game->img_db.exit);
}
